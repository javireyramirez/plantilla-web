...
import { useOrganizationForm } from '../model/use-organizations-detail';
...
export default function OrganizationDetail() {
  ...
  const { isEditing, organizationName, isLoading, form, handleSubmit, handleDelete, isPending } =
    useOrganizationForm(id);

  const tabs = [
    { value: 'detail', label: t('organizations.detail'), viewAtCreate: true },
    { value: 'audit', label: t('organizations.audit'), viewAtCreate: isEditing },
  ];
        ...
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink asChild className="transition-colors hover:text-foreground">
              <Link to="/organizations">{t('organizations.title')}</Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="font-medium text-foreground">
              {isEditing ? `${organizationName}` : t('organizations.createTitle')}
            </BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-card p-4 rounded-xl border shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground hidden sm:block">
            {isEditing ? t('organizations.editDescription') : t('organizations.createDescription')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          {isEditing && (
            <>
              ...
              {/* Nueva : Visible solo en pantallas muy grandes (xl) */}
              <Button
                type="button"
                variant="outline"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <Card className="lg:col-span-1 shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-semibold">
                  {t('organizations.basicData')}
                </CardTitle>
                <CardDescription>{t('organizations.identificationInfo')}</CardDescription>
              </CardHeader>
              <CardContent>
                <form
            ...
            <Card className="shadow-sm lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription>{t('organizations.pendingDefine')}</CardDescription>
              </CardHeader>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t('organizations.deleteConfirmDesc')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
